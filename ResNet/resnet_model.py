...
from dropblock import dropblock, dropblock2, dropblock3
...
def resnet_backbone(image, num_blocks, group_func, block_func, args):
    ...
    if args.dropblock_groups:
        ...
        for block_group in dropblock_groups:
            ...
            if args.strategy == 'decay':
                ...
                keep_probs[block_group - 1] = keep_prob 
    ...
    with argscope(Conv2D, use_bias=False, kernel_initializer=tf.variance_scaling_initializer(scale=2.0, mode='fan_out')):
        ...
        l = group_func('group0', l, block_func, 64 , num_blocks[0], 1, keep_prob=keep_probs[0], dropblock_size=args.blocksize)
        l = group_func('group1', l, block_func, 128, num_blocks[1], 2, keep_prob=keep_probs[1], dropblock_size=args.blocksize)
        l = group_func('group2', l, block_func, 256, num_blocks[2], 2, keep_prob=keep_probs[2], dropblock_size=args.blocksize)
        l = group_func('group3', l, block_func, 512, num_blocks[3], 2, keep_prob=keep_probs[3], dropblock_size=args.blocksize)
        l = GlobalAvgPooling('gap', l)
        logits = FullyConnected('linear', l, 1000, kernel_initializer=tf.random_normal_initializer(stddev=0.01))
    return logits


def resnet_group(name, l, block_func, features, count, stride, keep_prob, dropblock_size):
...
def resnet_bottleneck(l, ch_out, stride, keep_prob, dropblock_size, stride_first=False):
    ...
    """
    shortcut = l
    shortcut = dropblock2(shortcut, keep_prob=keep_prob, dropblock_size=dropblock_size)
    l = Conv2D('conv1', l, ch_out, 1, strides=stride if stride_first else 1, activation=BNReLU)
    l = dropblock2(l, keep_prob=keep_prob, dropblock_size=dropblock_size)
